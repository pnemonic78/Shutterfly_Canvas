...
import com.shutterfly.canvas.ui.carousel.Carousel
import com.shutterfly.canvas.ui.drag.DragScreen
import com.shutterfly.canvas.ui.drag.DragTarget
...
fun CanvasScreen(modifier: Modifier = Modifier) {
    ...
    DragScreen(modifier = modifier.fillMaxSize()) {
        Column(
            ...
            Box(
                ...
                DragTarget(
                    onDragDrop = { _, dropOffset -> viewModel.onCarouselImagesDropped(dropOffset) }
                ) { isDragHover ->
                    Canvas(
                        images = images,
                        isDragging = isDragHover,
                        onImageChange = viewModel::onCanvasImageChange
                    )
                }
            }
            Carousel(
